...
std::string Socket::getStringIP(const sockaddr_in& addr) {
  ...
  oss << static_cast<unsigned int>(a) << '.' << static_cast<unsigned int>(b) << '.'
      << static_cast<unsigned int>(c) << '.' << static_cast<unsigned int>(d);
  return oss.str();
}
...
bool Socket::bindSocket(const std::string& ip, int port, int& fd_listen, sockaddr_in& addr) {
