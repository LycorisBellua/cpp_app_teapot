...
namespace
{
	bool createSocket(int& fd_listen);
	bool bindSocket(const std::string& ip, int port, int& fd_listen,
		sockaddr_in& addr);
	bool resolveIPv4(const std::string& ip, sockaddr_in& out);
	bool listenForClients(int fd_listen);
}
...
namespace
{
	bool createSocket(int& fd_listen)
	{
		...
		else if (!Socket::makeFdNonBlocking(fd_listen))
			return false;
		return true;
	}
