...
std::string Response::compose(const Router& router, CookieJar* jar, Client& c) {
  ...
  bool should_close = c.shouldCloseConnection() || c.response_data->code == 400;
  ...
  HexColorCode::embedBackgroundColor(c.getBackgroundColor(), c.response_data->content);
  return Response::serialize(*c.response_data, is_head, should_close, cookie_headers);
}
...
void Response::setRouteInfoAndResponseData(const Router& router, Client& c) {
  ...
  if (!c.route_info) {
    ...
    if (c.route_info->error_code) {
      if (c.route_info->error_code == 301 || c.route_info->error_code == 302) {
        c.response_data = new ResponseData;
        c.response_data->code = c.route_info->error_code;
        c.response_data->headers.insert(std::make_pair<std::string, std::string>(
            "Location", c.route_info->location.redirect.second));
      }
      else if (c.route_info->error_code == 400 || c.route_info->error_code == 404) {
        c.response_data = new ResponseData(c.route_info->error_code);
      }
      else {
        c.response_data = new ResponseData(c.route_info->error_code, c.route_info->server.errors);
      }
      return;
    }
  }
  ...
  else if (c.route_info->request.method == "GET" || c.route_info->request.method == "HEAD") {
...
std::string Response::serialize(const ResponseData& res, bool is_head, bool should_close,
  ...
  if (res.code == 100) {
    ...
    str += getHeaderLine("Content-Length", Helper::nbrToString(res.content.length()));
...
std::string Response::getHeaderLine(const std::string& key, const std::string& value) {
