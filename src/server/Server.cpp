...
bool Server::addListener(const std::string& ip, int port) {
  ...
  if (fd_listen < 0 || !addListenerToEventHandler(fd_listen)) {
    Log::error(
        "Error: Server: addListener: can't create listener or add "
        "it to event handler");
    close(fd_listen);
    return false;
  }
  std::pair<std::map<int, Listener>::iterator, bool> result =
      listeners_.insert(std::make_pair(fd_listen, Listener(fd_listen, ip, port)));
  if (!result.second) {
...
bool Server::runEventLoop() {
  ...
  while (1) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if ((can_read || !c.isBufferEmpty()) && !c.isFullyParsed() && !c.parseRequest()) {
        closeConnection(fd);
        continue;
      }
