...
namespace
{
	...
	size_t findBodyTag(const std::string& html);
	size_t insertStyleAttributeIfMissing(std::string& html, size_t body_tag);
}
...
namespace HexColorCode
{
	bool isValid(const std::string& str)
	{
		...
		{
			if (std::islower(str[i]) || !isHexChar(str[i]))
				return false;
		}
		return true;
	}
...
namespace
{
	...
	size_t insertStyleAttributeIfMissing(std::string& html, size_t body_tag)
	{
		size_t attribute;
		if (html[body_tag + 5] == '>'
			|| (attribute = html.find("style=", body_tag + 5))
				== std::string::npos)
		{
