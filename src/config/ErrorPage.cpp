...
namespace
{
	...
	std::string getUserErrorPage(const int code,
...
namespace ErrorPage
{
	std::string get(const int code,
		const std::map<int, std::string>& user_specified)
	{
...
namespace
{
	...
	std::string getDefaultErrorPage(const int code)
	{
		...
		html << "<!DOCTYPE html><html><head><meta charset=\"UTF-8\">"
			...
			".error-box { border: 2px solid #ccc; border-radius: 8px; "
			"padding: 30px; background: #f9f9f9; text-align: center; }"
		...
		const std::map<int, std::string>& errors)
	{
		...
		if (found != errors.end())
		{
			std::pair<bool, std::string> html =
				Filesystem::readFile(found->second);
