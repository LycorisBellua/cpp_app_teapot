...
ResponseData::ResponseData(const int error_code)
	: code(error_code), code_msg(StatusMsg::get(error_code)),
	content(ErrorPage::get(error_code)), content_type("text/html")
{
}
...
ResponseData::ResponseData(const int error_code,
	...
	: code(error_code), code_msg(StatusMsg::get(error_code)),
	content(ErrorPage::get(error_code, user_errors)), content_type("text/html")
{
}
...
ResponseData::ResponseData(const int return_code, const std::string &content,
	...
	: code(return_code), code_msg(StatusMsg::get(return_code)),
