...
void debugPrintConfig(const Config& conf) {
  ...
  for (std::vector<ServerData>::iterator s = servers.begin(); s != servers.end(); ++s) {
    ...
    for (std::vector<LocationData>::iterator l = s->locations.begin(); l != s->locations.end(); ++l) {
      ...
      if (!l->allowed_methods.empty()) {
        ...
        for (std::vector<std::string>::iterator m = l->allowed_methods.begin(); m != l->allowed_methods.end(); ++m)
          std::cout << *m << " ";
        std::cout << "\n";
      }
